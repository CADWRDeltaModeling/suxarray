...
class Grid(ux.Grid):
    ...
    def __init__(self, dataset, **kwargs):
        ...
        if "start_index" in dataset.Mesh2_face_nodes.attrs:
            ...
            if start_index == 1:
                ...
                dataset.update({"Mesh2_face_nodes": dataset.Mesh2_face_nodes - 1})
                ...
                dataset.Mesh2_face_nodes.attrs["_FillValue"] = fill_value - 1
            elif start_index != 0:
    ...
    def face_polygons(self):
        if self._face_polygons is None:
            ...
            self._face_polygons = xr.apply_ufunc(
                create_polygon,
                self.Mesh2_face_nodes,
                input_core_dims=((self.Mesh2_face_nodes.dims[1],),),
                dask="parallelized",
    ...
    def node_points(self):
        if self._node_points is None:
            self._node_points = xr.apply_ufunc(
    ...
    def subset(self, polygon: Polygon):
        ...
        fill_value = self.Mesh2_face_nodes.attrs["_FillValue"]
        node_subset = np.unique(
            face_subset.where(face_subset >= 0, drop=True).values
        ).astype(int)
        ...
        # dictionary
        node_dict = dict(zip(node_subset, np.arange(len(node_subset))), dtype=np.int32)
        node_dict[-1] = -1
        new_edge_nodes = np.array(
            [[node_dict[n] for n in edge] for edge in edge_subset], dtype=np.int32
        )
        da_new_edge_nodes = xr.DataArray(
            new_edge_nodes,
        ...
        da_original_node_indices = xr.DataArray(
            node_subset,
            dims=("nMesh2_node",),
            attrs={"long_name": "Original node indices", "start_index": 0},
        )
    ...
    def compute_face_areas(self):
        ...
        """
        ret = xr.apply_ufunc(
            lambda v: np.vectorize(lambda x: x.area)(v),
            self.face_polygons,
            dask="parallelized",
            output_dtypes=float,
        )
...
def coerce_mesh_name(ds):
    ...
    dims_to_rename = {k: k.replace(name_org, "Mesh2") for k in ds.dims if name_org in k}
    ds = ds.rename_dims(dims_to_rename)
    # Rename coordinates
    # First, collect coordinates to rename
    coords_to_rename = {
        k: k.replace(name_org, "Mesh2") for k in ds.coords if name_org in k
    }
    ds = ds.rename(coords_to_rename)
    # Rename variables
    # First, collect variables to rename
    vars_to_rename = {
        k: k.replace(name_org, "Mesh2") for k in ds.data_vars if name_org in k
    }
...
def read_hgrid_gr3(path_hgrid):
    ...
    ds = xr.Dataset()
    ds["Mesh2_node_x"] = xr.DataArray(data=df_nodes[1].values, dims="nMesh2_node")
    ds["Mesh2_node_y"] = xr.DataArray(data=df_nodes[2].values, dims="nMesh2_node")
    # Replace NaN with -1
