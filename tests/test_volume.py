...
def test_calculate_prism_volume_synthetic_shaved_cells():
    ...
    # z shape: (n_time=2, n_node=5, n_layer=4) -> 3 interfaces.
    z = np.array(
        [
            [
                [-3.0, -2.0, -1.0, 0.0],
    ...
    # Expected shape: (n_time, n_interface, n_face)
    assert volumes.shape == (2, 3, 3)
    ...
    assert volumes[0, 0, 0] == pytest.approx(2.0 / 3.0)
    assert volumes[1, 2, 2] == pytest.approx(4.0)